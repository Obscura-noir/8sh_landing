# PRD: Лендинг 8sh.ru - Конверсионная страница международных платежей

## 1. Обзор проекта

### 1.1 Цель лендинга
Создать высококонверсионную landing page для платформы **8sh.ru**, которая эффективно конвертирует посетителей в пользователей через education-first подход, AI-персонализацию и решение критических болей пользователей в сфере международных платежей.

### 1.2 Основная гипотеза конверсии
**Если мы покажем прозрачность условий + AI-рекомендации + социальное доказательство + простоту использования, то конверсия visitor-to-lead составит 15-25%** (vs industry average 2-5%).
...
- ❌ Сложность для новичков → ✅ AI-ассистент + guided onboarding
- ❌ Нет urgency elements → ✅ Smart urgency + персонализация
- ❌ Ограниченный FAQ → ✅ Comprehensive educational hub
- ❌ Manual выбор провайдеров → ✅ AI-рекомендации оптимальных маршрутов
...
│   currency      │  от ИИ за 30 секунд                  │
│   symbols]      │                                       │
│                 │  ✅ Комиссии от 0.5%                 │
│                 │  ✅ Переводы от 10 минут              │
│                 │  ✅ AI подбирает лучший маршрут       │
│                 │  ✅ 50+ валют и способов              │
│                 │                                       │
...
┌─────────────────────────────────────────────────────────┐
│ 🧮 ВСТРОЕННЫЙ AI-КАЛЬКУЛЯТОР                           │
│                                                         │
│ Откуда: [Россия ▼]  Сумма: [10,000] [USD ▼]           │
│ Куда:   [Китай ▼]                                      │
│                                                         │
│ 🤖 AI анализирует 50+ способов...                      │
│                                                         │
│ ┌─ ТОП-3 РЕКОМЕНДАЦИИ ─────────────────────────────────┐ │
...
"Международные платежи с AI-оптимизацией"

**Поддерживающий value prop (подзаголовок H2):**
"Сравните 50+ способов перевода и получите персональную рекомендацию от ИИ за 30 секунд"

**Обоснование выбора:**
- **"AI-оптимизация"** - уникальный дифференциатор vs RealPay
...
• AI находит самый выгодный маршрут за 30 секунд
...
• AI-рекомендации (не гадаете на кофейной гуще)
...
- "Создать заявку на перевод" (Основное действие)
- "Начать работу" (Full signup)
- "Связаться с менеджером" (Enterprise)

**Принципы CTA копирайтинга:**
- Использовать активные глаголы ("Получить", "Создать", "Сравнить")
- Указывать конкретную выгоду ("AI-рекомендацию", "стоимость")
...
### 5.3 AI-калькулятор реализация

```typescript
// Компонент AI-калькулятора
...
const AICalculator: React.FC = () => {
  ...
  const calculateWithAI = async (params: CalculatorState) => {
    setIsLoading(true);
    
    // Вызов API для получения AI-рекомендаций
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-semibold mb-4">
        🤖 AI-калькулятор международных переводов
      </h3>
      
      {/* Форма ввода */}
      <CalculatorForm 
        onSubmit={calculateWithAI}
        loading={isLoading}
      />
...
```html
<!-- Meta tags -->
<title>Международные переводы с AI | 8sh.ru - Сравните 50+ способов</title>
<meta name="description" content="Получите AI-рекомендацию лучшего способа международного перевода за 30 секунд. Комиссии от 0.5%, 50+ валют, проверенные провайдеры.">

<!-- Open Graph -->
<meta property="og:title" content="Международные переводы с AI-оптимизацией">
<meta property="og:description" content="AI находит самый выгодный способ перевода за 30 секунд">
...
<script type="application/ld+json">
{
  ...
  "name": "8sh.ru",
  "description": "Платформа международных переводов с AI-оптимизацией",
...
const heroHeadlineTest = {
  ...
  variants: {
    A: 'Международные платежи с AI-оптимизацией',
    B: 'Найдите лучший способ перевода за 30 секунд с помощью ИИ',
...
const ctaButtonTest = {
  ...
  variants: {
    A: { text: 'Получить AI-рекомендацию', color: 'blue', size: 'large' },
...
- ✅ Статический калькулятор без AI
...
**Фаза 3: AI и Персонализация (2-3 недели)**
- 🔮 AI-рекомендации в калькуляторе
...
**AI Features Критерии:**
- [ ] AI-рекомендации работают корректно
...
**Пользовательские:**
- **AI-differentiation:** Уникальные AI-рекомендации vs конкуренты
...
*Этот PRD является живым документом и должен обновляться по мере получения новых данных от пользователей и изменения рыночных условий. Все технические решения оптимизированы для максимальной конверсии и user experience.*